...
const Viewprize = () => {
    ...
    useEffect(() => {
          fetch(`http://3.87.25.45/winnn/data/${email}`)
          .then(res => res.json())
      ...
      fetch(`http://3.87.25.45/gif/won/data/${email1}`)
      .then(res => res.json())
      ...
      const Submitt = (e) =>{
        e.preventDefault();
        axios.post('http://3.87.25.45/check/won/data',{email1})
        .then(res => {
            if(res.data.Status === "None"){
                e.preventDefault();
                axios.post('http://3.87.25.45/gif/won/data',{gifno, gifname,gifdisc, giftime, email1, gifimgurl,rank, })
                .then(res => {
