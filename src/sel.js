...
const Sel = () => {
  ...
  const HandleSubmit = async (e) => {
    e.preventDefault();
    axios.post('http://3.87.25.45/way/email', {email})
    .then(res => {
      if(res.data.Status === "BAD"){
        axios.post('http://3.87.25.45/validtr/id', {trID})
        .then(res => {
          if(res.data.Status === "BAD"){
            e.preventDefault();
        axios.post('http://3.87.25.45/trid/id', {trID})
        .then(res => {
          if(res.data.Status === "OK"){
            axios.post("http://3.87.25.45/select",{way,email, ip})
            .then(res => {
            if(res.data.Status === "OK") {
              axios.post("http://3.87.25.45/way01",{way,email, ip})
              .then(resp => {
                if(resp.data.Status === "OK"){
                  axios.post('http://3.87.25.45/validid/post',{email, valid, trID})
                  .then(ress => {
