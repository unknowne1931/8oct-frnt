  ...
  const location = useLocation()
    ...
    useEffect(() => {
      fetch(`http://3.87.25.45/way01/${email}`)
        ...
        fetch(`http://3.87.25.45/won/length`)
        .then(res => res.json())
        .then(info => setCount(info))

        fetch('http://3.87.25.45/prize/data')
        .then(res => res.json())
    ...
    const Elim1 = async (e) => {
      const response = await fetch(`http://3.87.25.45/way01/put/${id}`, {
            method: 'PUT',
            headers: {
    ...
    function Won(){
      axios.post('http://3.87.25.45/won/id/check',{email})
      .then(res => {
        if(res.data.Status === "OK"){
          window.location.href = `/profile?email=${email}`
        }else{
          axios.post("http://3.87.25.45/won/match" ,{email , IP})
          .then(res => {
            if(res.data.Status === "OK"){
              {prize.map((user) =>{
                if(count.dataLength <= user.total){
                  ...
                  axios.post("http://3.87.25.45/winnn",{email, IP ,star, no, username})
                  .then(res => {
                    if(res.data.Status === "OK"){
                  ...
                  const star = "6";
                  const no = count.dataLength;
                  axios.post("http://3.87.25.45/winnn",{email, IP ,star,no})
                  .then(res => {
                    if(res.data.Status === "OK"){
                      window.location.href=`/prize?email=${email}&no=${count.dataLength}`
                    }
