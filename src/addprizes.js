...
const Addprizes = () => {
    ...
    useEffect(() => {
        ...
        fetch("http://3.87.25.45/amount/dataLength")
        .then(res => res.json())
    ...
    const handleSubmitt = async (e) => {
        e.preventDefault();
        try {
          const response = await fetch('http://3.87.25.45/gif/data', {
            method: 'POST',
            headers: {
      ...
      const Subbbit = (e) =>{
        e.preventDefault();
        axios.post("http://3.87.25.45/amount/data/fix" , {email, amount})
        .then(res => {
      ...
      const Subbbitupdate = (e) => {
        e.preventDefault();
        axios.put(`http://3.87.25.45/amount/data/put/${email}`, {amount})
        .then(res => {
          if(res.data.Status === "OK"){
            window.location.href = '/payment'
          }
